...
void handle_network_to_transport(void* network_payload) {
    if(network_payload == NULL) {
        fprintf(stderr, "TP Error: Received NULL data pointer from network layer.\n");
        return;
    }
    ...
    if(true) {
        ...
        if(DEBUG_ENABLED) printf("TP: Received UDP segment. Src Port: %u, Dest Port: %u, Length Field: %u\n", src_port, dest_port, udp_length);
        if(udp_length < header_size) {
            fprintf(stderr, "TP Error: UDP header length (%u) < UDP header struct size (%zu). Discarding.\n", udp_length, header_size);
            free(network_payload);
            return;
        }
        // if(checksum != 0) { /* Calculate and verify checksum */ }
        ...
        if(app_payload) {
            ...
            if(thpool != NULL) {
                if(thpool_add_work(thpool, (void (*)(void*))handle_transport_to_application, app_payload) != 0) {
                    fprintf(stderr, "TP Error: Failed to add task to thread pool for Application Layer.\n");
                    free(app_payload);
                }
                else if(DEBUG_ENABLED) printf("TP: UDP Payload (Size: %zu) passed to thread pool for APP processing.\n", app_payload_size);
            }
            else {
                fprintf(stderr, "TP Error: Thread pool is NULL when trying to add APP work.\n");
                free(app_payload);
            }
        }
        else fprintf(stderr, "TP Error: Failed to allocate memory for application payload.\n");
    }
    else if(DEBUG_ENABLED) fprintf(stderr, "TP Error: Received data block too small for UDP header.\n");
    free(network_payload);
}
...
int handle_application_to_transport(const unsigned char* app_data, size_t app_data_length, uint16_t src_port, uint16_t dest_port) {
    if(app_data == NULL && app_data_length > 0) {
        fprintf(stderr, "TP Error: Send request from application with NULL data but positive length (%zu).\n", app_data_length);
        return -1;
    }
    size_t udp_header_size = sizeof(simple_udp_header_t);
    size_t udp_segment_length = udp_header_size + app_data_length;
    if(DEBUG_ENABLED) printf("TP: Sending %zu bytes of app data from Port %u to Port %u.\n", app_data_length, src_port, dest_port);
    unsigned char* udp_segment = (unsigned char*)malloc(udp_segment_length);
    if(!udp_segment) {
        fprintf(stderr, "TP Error: Failed to allocate memory for UDP segment.\n");
        return -1;
    }
    simple_udp_header_t* udp_header = (simple_udp_header_t*)udp_segment;
    ...
    if(DEBUG_ENABLED) printf("TP: UDP Segment created (Total Length: %zu).\n", udp_segment_length);
    if(handle_transport_to_network(udp_segment, udp_segment_length, UDP_PROTOCOL_NUMBER) != 0) {
        fprintf(stderr, "TP Error: Network layer failed to send UDP segment.\n");
        free(udp_segment);
        return -1;
    }
    free(udp_segment);
    if(DEBUG_ENABLED) printf("TP: UDP Segment successfully sent to network layer.\n");
    return 0;
}